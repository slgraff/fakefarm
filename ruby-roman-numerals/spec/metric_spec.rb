require 'metric'
require_relative 'fixtures/metric_lengths.rb'
...
RSpec.describe Metric do
  ...
  subject { Metric.new(CHART, TRANSLATIONS) }
  ...
  METRIC_LENGTHS.each do |from, to|
    from = "#{from[from.keys[0]]}#{from.keys[0]}"
    unit = to.keys[0]
    to = "#{to[to.keys[0]]}#{to.keys[0]}"

    it "#{from} to #{to}" do
      expect(subject.from(from).to("#{unit}")).to eq(to)
    end
  end
  ...
  #     expect { Metric.new.to }.to raise_error('Please supply conversions')
  #   end
  # end
